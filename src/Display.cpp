...
void Display::RunSetup()
{
  ...
  #ifdef HAS_SCREEN
    // *****************************************************************
    // *** INSERINDO SEUS NOVOS DADOS DE CALIBRAÇÃO (3.5" Resistiva) ****
    // *****************************************************************
    uint16_t calData[5] = { 339, 3400, 295, 3542, 6 }; // <--- SEUS NOVOS VALORES APLICADOS

    #if !defined(CYD_32CAP) && !defined(CYD_35CAP)
      tft.setTouch(calData); // Linha ATIVADA para carregar sua calibração
    #endif
  #endif
...
void Display::setupScrollArea(uint16_t tfa, uint16_t bfa) {
  // CORREÇÃO DEFINITIVA: Força o uso do ST7796
